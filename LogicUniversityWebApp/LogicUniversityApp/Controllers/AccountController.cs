...
namespace LogicUniversityApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> Assign(FormCollection form)
        {
            string userId = form["userId"].ToString();
            string role = form["UserRoles"].ToString();
            if (userId != null && role != null)
            {
                var roles = await UserManager.GetRolesAsync(userId);
                await UserManager.RemoveFromRolesAsync(userId, roles.ToArray());
                await this.UserManager.AddToRoleAsync(userId, role);
            }

            ViewBag.id = userId;
            ViewBag.roles = new SelectList(context.Roles.Where(x => x.Name != "Admin").ToList(), "Name", "Name");
            ViewBag.users = context.Users.ToList();
            return View("~/Views/Account/ShowUser.cshtml");
        }
